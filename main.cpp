...
void trace(Scene& scene, Ray& ray, size_t depth, vec4f& color)
{
	...
	vec4f temp_color = scene.ambient_light;
	...
	for (auto& light : scene.point_lights)
	{
		...
		// Diffuse
		temp_color = mul4f(temp_color, mul4f(material.diffuse, div4fs(mul4sf((-dot4f(normalize4f(shadow_ray.direction), norm)), light.intensity), dot4f(shadow_ray.direction, shadow_ray.direction))));
		// Specular TODO:
...
void render_camera(Scene& scene, int camIndex)
{
	...
	for (int i = 0; i < current_camera.image_width; i++)
	{
		for (int j = 0; j < current_camera.image_height; j++)
		{
			...
			image[i * current_camera.image_height + j] = scene.background_color;
